...
export default {
    ...
    list: async ( req, res, next ) => {
        try {
            ...
            const reg = await models.Venda.find({ 
                $or: [
    ...
    update: async (req, res, next) => {
        try {
            const reg = await models.Venda.findByIdAndUpdate( { _id: req.body._id }, { name: req.body.name, description: req.body.description } );
            res.status(200).json(reg);
        } catch (error) {
            res.status(500).send({
                message: 'Ocorreu um erro!'
    ...
    */
    activate: async (req, res, next) => {
        try {
            const reg = await models.Venda.findByIdAndUpdate({ _id: req.body._id }, { status: 1 });
            // Atualizar stock
    ...
    deactivate: async (req, res, next) => {
        try {
            const reg = await models.Venda.findByIdAndUpdate({ _id: req.body._id }, { status: 0 });
            // Atualizar stock
            let details = reg.details;
            details.map(function(x){
