<template>
  <v-layout align-start>
    <v-flex>
              ...
              <v-container>
                <v-row>
                  <v-flex xs12 sm12 md12 lg12 xl12>
                    <v-text-field @keyup.enter="listarArtigos" class="text-xs-center" v-model="texto" append-icon="search" label="Pesquisa">
                    </v-text-field>
                    ...
                    <template>
                      <v-data-table :headers="cabeceraArtigos" :items="artigos" class="elevation-1" >
                        ...
                        <template v-slot:item.action="{ item }">
                          <v-icon small @click="adicionarDetalhe(item)" >
                            add_box
                          </v-icon>
                        </template>
      ...
      <v-data-table :headers="headers" :items="compras" :search="search" class="elevation-1" v-if="verNovo==0" >  
        <template v-slot:item.action="{ item }">
